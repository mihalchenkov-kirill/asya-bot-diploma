...
async def _(callback: types.CallbackQuery):
    await callback.message.answer_photo(
        ...
        reply_markup=get_mixed_buttons(
            buttons={'Далее': 'extra_next_3',
                     'Кризисная психологическая служба для детей и подростков': 'https://szgmu.ru/'}, sizes=(1, 1)),
    )
...
async def _(callback: types.CallbackQuery):
    await callback.message.answer_photo(
        ...
        reply_markup=get_mixed_buttons(
            buttons={'Далее': 'extra_next_4',
                     'Социальный проект «Без паники»': 'https://bez-paniki.online/'}, sizes=(1, 1)),
    )
